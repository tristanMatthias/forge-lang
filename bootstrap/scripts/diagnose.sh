...
RUN MODES
  ...
  --emit-regen-expr    ps3t.6.7 Step G: regenerate the CHECKED-IN production
                       expression static parser (parse/gen_expr_parser.av) from
                       the §2 grammar. Run after editing avra_grammar.av; commit
                       the result (the regen guard test fails until you do).
...
# ps3t.6.7 Step G — regenerate the CHECKED-IN production expression static parser
...
mode_emit_regen_expr() {
  ...
  mkdir -p "$dir"
  local driver="$dir/regen_expr_driver.av"
  cat > "$driver" <<'AVEOF'
// GENERATED by diagnose.sh --emit-regen-expr (ps3t.6.7 Step G). Writes the
// production expression static parser from emit_expr_parser_file().
use @std.avrac.features.grammar.{emit_expr_parser_file}
use @std.avrac.core.{avra_selfhost_write_file}

fn main() {
    let src = emit_expr_parser_file()
    let _ = avra_selfhost_write_file("packages/std-avrac/src/parse/gen_expr_parser.av", src)
    println("wrote parse/gen_expr_parser.av (${string(src.length)} bytes)")
}
AVEOF
  log "emit-regen-expr: rendering the production expression static parser via emit.av"
  run_fg "$driver" || die "emit-regen-expr: driver (emit_expr_parser_file render) failed"
  [ -f "packages/std-avrac/src/parse/gen_expr_parser.av" ] || die "emit-regen-expr: driver did not write gen_expr_parser.av"
  ok "emit-regen-expr: parse/gen_expr_parser.av regenerated from the §2 grammar"
}

# ps3t.6.7 Phase 2 — regenerate the CHECKED-IN production DECLARATION static parser
# (packages/std-avrac/src/parse/gen_decl_parser.av) from avra_decl_dispatch_grammar.
...
mode_emit_regen_decl() {
  ...
  local dir="$BUILD_DIR/emit_gen"
  mkdir -p "$dir"
  local driver="$dir/regen_decl_driver.av"
  cat > "$driver" <<'AVEOF'
// GENERATED by diagnose.sh --emit-regen-decl (ps3t.6.7 Phase 2). Writes the
// production declaration static parser from emit_decl_parser_file().
use @std.avrac.features.grammar.{emit_decl_parser_file}
use @std.avrac.core.{avra_selfhost_write_file}

fn main() {
    let src = emit_decl_parser_file()
    let _ = avra_selfhost_write_file("packages/std-avrac/src/parse/gen_decl_parser.av", src)
    println("wrote parse/gen_decl_parser.av (${string(src.length)} bytes)")
}
AVEOF
  log "emit-regen-decl: rendering the production declaration static parser via emit.av"
  run_fg "$driver" || die "emit-regen-decl: driver (emit_decl_parser_file render) failed"
  [ -f "packages/std-avrac/src/parse/gen_decl_parser.av" ] || die "emit-regen-decl: driver did not write gen_decl_parser.av"
  ok "emit-regen-decl: parse/gen_decl_parser.av regenerated from the §2 grammar"
}

# t-47hc.4.4 (Phase 3d) — regenerate the CHECKED-IN production TYPE static parser
...
mode_emit_regen_type() {
  ...
  local dir="$BUILD_DIR/emit_gen"
  mkdir -p "$dir"
  local driver="$dir/regen_type_driver.av"
  cat > "$driver" <<'AVEOF'
// GENERATED by diagnose.sh --emit-regen-type (t-47hc.4.4 Phase 3d). Writes the
// production type static parser from emit_type_parser_file().
use @std.avrac.features.grammar.{emit_type_parser_file}
use @std.avrac.core.{avra_selfhost_write_file}

fn main() {
    let src = emit_type_parser_file()
    let _ = avra_selfhost_write_file("packages/std-avrac/src/parse/gen_type_parser.av", src)
    println("wrote parse/gen_type_parser.av (${string(src.length)} bytes)")
}
AVEOF
  log "emit-regen-type: rendering the production type static parser via emit.av"
  run_fg "$driver" || die "emit-regen-type: driver (emit_type_parser_file render) failed"
  [ -f "packages/std-avrac/src/parse/gen_type_parser.av" ] || die "emit-regen-type: driver did not write gen_type_parser.av"
  ok "emit-regen-type: parse/gen_type_parser.av regenerated from the type grammar"
}
...
mode_emit_regen_pat() {
  ...
  local dir="$BUILD_DIR/emit_gen"
  mkdir -p "$dir"
  local driver="$dir/regen_pat_driver.av"
  cat > "$driver" <<'AVEOF'
// GENERATED by diagnose.sh --emit-regen-pat (t-47hc.4.4 Phase 3d). Writes the
// production pattern static parser from emit_pat_parser_file().
use @std.avrac.features.grammar.{emit_pat_parser_file}
use @std.avrac.core.{avra_selfhost_write_file}

fn main() {
    let src = emit_pat_parser_file()
    let _ = avra_selfhost_write_file("packages/std-avrac/src/parse/gen_pat_parser.av", src)
    println("wrote parse/gen_pat_parser.av (${string(src.length)} bytes)")
}
AVEOF
  log "emit-regen-pat: rendering the production pattern static parser via emit.av"
  run_fg "$driver" || die "emit-regen-pat: driver (emit_pat_parser_file render) failed"
  [ -f "packages/std-avrac/src/parse/gen_pat_parser.av" ] || die "emit-regen-pat: driver did not write gen_pat_parser.av"
  ok "emit-regen-pat: parse/gen_pat_parser.av regenerated from the pattern grammar"
}
...
mode_emit_gen_check() {
  ...
  cat > "$driver" <<'AVEOF'
// GENERATED by diagnose.sh --emit-gen-check (ps3t.6.5.11). Runs emit.av over
...
use @std.avrac.features.grammar.{avra_expr_grammar, avra_stmt_flip_grammar, avra_type_grammar, avra_program_grammar, emit_parser_source, grammar_with_builder_kinds, builder_kind_rows}
...
fn harness_block() -> string {
    ...
    "}\n\n" +
    "fn main() {\n" +
    "    let ok = agree(\"42\") && agree(\"1 + 2\") && agree(\"1 + 2 * 3\") && agree(\"1 * 2 + 3 * 4\") &&\n" +
    "        agree(\"1 - 2 - 3\") && agree(\"(1 + 2) * 3\") && agree(\"((1))\") &&\n" +
    "        agree(\"-5\") && agree(\"~5\") && agree(\"!~-1\") && agree(\"~a + 2\") &&\n" +
    "        agree(\"1 && 2 || 3\") && agree(\"1 | 2 & 3\") && agree(\"1 << 4\") && agree(\"7 % 3\") &&\n" +
    "        agree(\"1 | 2 ^ 3 & 4 == 5 < 6 << 7 + 8 * 9 % 2\") &&\n" +
    "        agree(\"x\") && agree(\"foo + bar\") && agree(\"\\\"hi\\\"\") &&\n" +
    "        agree(\"a.b\") && agree(\"a[0]\") && agree(\"a.b.c\") && agree(\"a[0][1]\") && agree(\"-a.b\") &&\n" +
    "        agree(\"f()\") && agree(\"f(1)\") && agree(\"f(1, 2, 3)\") && agree(\"f(1)(2)\") &&\n" +
    "        agree(\"a.b(c).d(e)\") && agree(\"f(g(x), h(y, z))\")\n" +
...
fn pat_imports_block() -> string {
    "use @std.avrac.features.grammar.{Token, CapVal, PState, new_pstate, capval_to_pat, capval_to_pat_list, capval_tok_text, pat_num_node, pat_bool_node, pat_str_node, ident_pat_bare, param_entries_from_toks, run_grammar_pat, text_is_word, _gb_build_defer, _gb_build_errdefer, _gb_build_when_arm, _gb_build_when_default}\n" +
    "use @std.avrac.parse.{parser_new}\n" +
    "use @std.avrac.features.match.{avra_pat_grammar}\n" +
...
fn pat_harness_block() -> string {
    ...
    "}\n\n" +
    "fn agree(src: string) -> bool { gen_pat(src) == hand_pat(src) }\n\n" +
    "fn main() {\n" +
    "    let ok = agree(\"_\") && agree(\"rest\") && agree(\"42\") && agree(\"-5\") && agree(\"3.14\") &&\n" +
...
fn type_imports_block() -> string {
    "use @std.avrac.features.grammar.{Token, CapVal, PState, new_pstate, capval_to_texpr, capval_to_texpr_list, capval_present, capval_tok_text, lookahead_ok, fold_type_union, mk_named_type, mk_splice_type, mk_qualified_type, mk_paren_type, mk_fn_type, mk_dyn_type, text_is_word, _gb_build_defer, _gb_build_errdefer, _gb_build_when_arm, _gb_build_when_default}\n" +
    "use @std.avrac.parse.{parser_new}\n" +
...
fn type_harness_block() -> string {
    ...
    "    }\n" +
    "}\n\n" +
    "fn agree(src: string) -> bool { gen_type(src) == hand_type(src) }\n\n" +
    "fn main() {\n" +
    "    let ok = agree(\"int\") && agree(\"string\") && agree(\"Foo\") && agree(\"MyType\") &&\n" +
...
fn program_imports_block() -> string {
    "use @std.avrac.features.grammar.{Token, CapVal, PState, new_pstate, capval_to_expr, capval_to_stmt, capval_to_texpr, capval_to_pentry, capval_to_field, capval_to_variant, capval_to_tparam, capval_tok_text, capval_tok_list, capval_present, literal_node, lookahead_ok, modes_with_flag_on, mk_binary, fold_binary_expr, fold_logical_expr, fold_nullish_expr, fold_postfix_expr, unary_expr, mk_assign_expr, fold_pipe_expr, rebind_eq_expr, mk_is_expr, mk_inlist_expr, mk_inrange_expr, mk_eqchain_expr, mk_channel, tok_starts_expr_leaf, fold_type_union, mk_named_type, mk_splice_type, mk_qualified_type, mk_paren_type, mk_dyn_type, mk_fn_type, mk_group_or_empty, mk_tuple_fr, mk_lambda, mk_lambda_typed, mk_block, mk_while, mk_if, mk_for_loop, mk_return_opt, mk_fn_decl, mk_fn_decl_g, mk_type_decl, mk_type_decl_any, mk_type_param, mk_enum_decl, mk_enum_decl_g, mk_shape_decl, mk_extern_fn, mk_trait_decl, mk_impl, mk_use, mk_use_pkg, mk_module, mk_field, mk_evariant, mk_param, mk_assoc_type, mk_trait_method, mk_method, mk_field_init, mk_with_suffix, capval_to_finit, capval_to_tparam, mk_map_lit, mk_qualified, capval_toks, mk_template, mk_doc_annotated, capval_is_error_expr, capval_is_error_stmt, error_stmt_of, mk_assoc_type_def, mk_fn_decl_where_g, mk_plain_name, mk_splice_name, mk_where_param, gensym_token, mk_let_binding_g, mk_quote, mk_expr_block, mk_splice, modes_with_counter_bumped, text_is_word, dump_expr, fold_postfix_with_expr, ident_pat_bare, mk_call_arg, mk_call_suffix, mk_catch_suffix, mk_enum_ctor_expr, mk_enum_ctor_suffix, mk_field_access, mk_force_unwrap, mk_generic_call_suffix, mk_grammar_block, mk_index_suffix, mk_list_comp, mk_list_lit, mk_opt_chain, mk_opt_method_call, mk_splice_struct, mk_struct_lit, mk_table_lit, mk_table_row, mk_tagged_template, mk_try_suffix, mk_tuple_index, mk_type_decl_g, mk_when_arm, mk_when_default, modes_with_flag_off, param_entries_from_toks, pat_bool_node, pat_num_node, pat_str_node, rawbrace_scan, tok_starts_pattern, tok_starts_type, capval_to_ann, capval_to_select_arm, capval_to_arm, capval_to_when_arm, capval_to_pat, capval_to_comp_config, capval_to_comp_config_pair, capval_to_comp_config_pair_list, capval_to_children_slot, capval_tok_texts, base_pred, mk_ann_g, mk_annotated_args_g, mk_arm_return, mk_arm, mk_children_slot_g, mk_children_slot_list, mk_comp_body_marker, mk_comp_config_g, mk_comp_config_list, mk_comp_config_pair_g, mk_comp_field_g, mk_component_block_g, mk_component_def_g, mk_component_paren_def_g, mk_const_binding_g, mk_export_decl, mk_for_binding, mk_for_destructure, mk_given_block_g, mk_if_expr, mk_if_let_stmt, mk_if_let, mk_isolated, mk_let_destructure_g, mk_match_expr, mk_match_table_expr, mk_match_table, mk_match, mk_method_shorthand_g, mk_module_doc, mk_none_cap, mk_parallel_g, mk_quote_arm, mk_quote_type, mk_raw_component_block_g, mk_select_arm_g, mk_select_g, mk_spawn, mk_spec_block_g, mk_splice_arm, mk_stamp_stmt_at, mk_table_header, mk_then_block_g, mk_trait_list, mk_when, _gb_build_defer, _gb_build_errdefer, _gb_build_when_arm, _gb_build_when_default}\n" +
    "use @std.avrac.parse.{parse_program_source}\n" +
    "use @std.avrac.core.{Expr, ExprId, Stmt, StmtId, TypeExpr, ParamEntry, FieldEntry, FieldInit, Variant, ValueType, NodeStore, render_stmt_ids, pattern_optional_present, pattern_optional_absent}\n\n"
}
...
fn main() {
    let rows = builder_kind_rows(feature_builders())
    let expr_src = imports_block() + emit_parser_source(grammar_with_builder_kinds(avra_expr_grammar(), rows)) + "\n" + harness_block()
    let _ = avra_selfhost_write_file("build/emit_gen/generated_parser.av", expr_src)
    let stmt_src = stmt_imports_block() + emit_parser_source(grammar_with_builder_kinds(avra_stmt_flip_grammar(), rows)) + "\n" + stmt_harness_block()
    let _ = avra_selfhost_write_file("build/emit_gen/generated_stmt_parser.av", stmt_src)
    let pat_src = pat_imports_block() + emit_parser_source(grammar_with_builder_kinds(avra_pat_grammar(), rows)) + "\n" + pat_harness_block()
    let _ = avra_selfhost_write_file("build/emit_gen/generated_pat_parser.av", pat_src)
    let type_src = type_imports_block() + emit_parser_source(grammar_with_builder_kinds(avra_type_grammar(), rows)) + "\n" + type_harness_block()
    let _ = avra_selfhost_write_file("build/emit_gen/generated_type_parser.av", type_src)
    let prog_src = program_imports_block() + emit_parser_source(grammar_with_builder_kinds(avra_program_grammar(), rows)) + "\n" + program_harness_block()
...
main() {
  ...
  case "$mode" in
    ...
    --emit-regen-type)    mode_emit_regen_type "$@" ;;
    --emit-regen-pat)     mode_emit_regen_pat "$@" ;;
