...
fn test_undefined_var_error() {
    ...
    assert!(
        stderr.contains("F0020") || stderr.contains("E0010"),
        "expected error code F0020 or E0010, got:\n{}",
        stderr
    );
}
...
fn test_immutable_assign_error() {
    ...
    assert!(
        ...
        "expected immutable assign error, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("F0013") || stderr.contains("E0013"),
        "expected error code F0013 or E0013, got:\n{}",
        stderr
    );
}
...
fn test_missing_brace_error() {
    ...
    assert!(
        stderr.contains("expected") && stderr.contains("RBrace"),
        "expected missing brace error, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("F0001") || stderr.contains("E0001"),
        "expected error code F0001 or E0001, got:\n{}",
        stderr
    );
}
...
fn test_multi_error_reporting() {
    ...
    assert!(
        stderr.contains("F0020"),
        "should contain undefined var error, got:\n{}",
        stderr
    );
    ...
    assert!(
        ...
        "summary should mention errors, got:\n{}",
        stderr
    );
}
...
fn test_did_you_mean_suggestion() {
    ...
    assert!(
        stderr.contains("F0020"),
        "should contain error code F0020, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("did you mean 'count'"),
...
fn test_unused_variable_warning() {
    ...
    assert!(
        stderr.contains("F0801"),
        "should contain unused var warning F0801, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("unused variable 'unused_var'"),
        "should name the unused var, got:\n{}",
        stderr
    );
}
...
fn test_unused_variable_underscore_suppressed() {
    ...
    assert!(
        !stderr.contains("F0801"),
        "underscore prefix should suppress warning, got:\n{}",
        stderr
    );
}
...
fn test_type_mismatch_suggestion() {
    ...
    assert!(
        stderr.contains("F0012"),
        "should contain error code F0012, got:\n{}",
        stderr
    );
    assert!(
    ...
    assert!(
        stderr.contains("string(value)"),
        "should suggest string(value), got:\n{}",
        stderr
    );
}
...
fn test_build_profile_human() {
    ...
    let stderr = strip_ansi(&String::from_utf8_lossy(&output.stderr));
    assert!(
        stderr.contains("Build Profile"),
        "should show profile header, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("lex"),
        "should show lex stage, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("parse"),
        "should show parse stage, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("codegen"),
        "should show codegen stage, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("Total:"),
        "should show total time, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("Functions:"),
        "should show function count, got:\n{}",
        stderr
    );
...
fn test_error_diff() {
    ...
    assert!(
        ...
        "should show fixed errors, got:\n{}",
        stdout
    );
    assert!(
        stdout.contains("F0020"),
        "should mention fixed F0020, got:\n{}",
        stdout
    );
    assert!(
        stdout.contains("Remaining"),
        "should show remaining errors, got:\n{}",
        stdout
    );
    assert!(
        stdout.contains("50%"),
        "should show 50% progress, got:\n{}",
        stdout
    );
...
fn test_autofix_type_mismatch() {
    ...
    assert!(
        stderr.contains("Applied") || stderr.contains("applied") || stderr.contains("fix"),
    ...
    assert!(
        fixed.contains("let port: int = 8080"),
        "should fix type annotation to int, got:\n{}",
        fixed
    );
...
fn test_because_chain_type_mismatch() {
    ...
    assert!(
        stderr.contains("F0012"),
        "should contain F0012, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("type mismatch"),
        "should contain type mismatch, got:\n{}",
        stderr
    );
    // Should show the origin of the type
    assert!(
        stderr.contains("get_name") || stderr.contains("returns string"),
...
fn test_wrong_arg_count_error() {
    ...
    assert!(
        stderr.contains("F0014"),
        "should contain error code F0014, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("expected 2") || stderr.contains("2 argument"),
        "should mention expected arg count, got:\n{}",
        stderr
    );
    assert!(
        stderr.contains("add(") || stderr.contains("example"),
...
fn test_runtime_assert_with_location() {
    ...
    let build_result = Command::cargo_bin("compiler")
        ...
        .args([
            ...
            "-o",
            std::env::temp_dir()
                .join("forge_test_runtime_assert")
                .to_str()
                .unwrap(),
    ...
    let output =
        std::process::Command::new(std::env::temp_dir().join("forge_test_runtime_assert")).output();

    if let Ok(output) = output {
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            stderr.contains("assertion failed"),
            "should contain assertion failure, got:\n{}",
            stderr
        );
        ...
        assert!(
            stderr.contains("line 2")
                || stderr.contains(":2")
                || stderr.contains("forge_test_runtime_assert.fg"),
