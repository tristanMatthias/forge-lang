...
fn base_url(config: &AiConfig) -> String {
    ...
    match config.package {
        PackageKind::Anthropic => "https://api.anthropic.com".to_string(),
        PackageKind::OpenAI => "https://api.openai.com".to_string(),
        PackageKind::Ollama => "http://localhost:11434".to_string(),
        PackageKind::OpenRouter => "https://openrouter.ai/api".to_string(),
    }
}
...
fn call_openai_compatible(
    ...
    let url = match config.package {
        PackageKind::Ollama => format!("{}/v1/chat/completions", base_url(config)),
        PackageKind::OpenRouter => format!("{}/v1/chat/completions", base_url(config)),
...
fn stream_ai(
    ...
    match config.package {
        PackageKind::Anthropic => stream_anthropic(config, model, messages, system),
            ...
            if let Some(arr) = results.as_array() {
                for result in arr {
                    ...
                    if let Some(call_id) = result["call_id"].as_str() {
                        if config.package == PackageKind::Anthropic {
...
/// Configure the global AI package
#[no_mangle]
pub extern "C" fn forge_ai_configure(
    ...
    let package_str = c_str_to_string(package);
    ...
    let kind = match package_str.to_lowercase().as_str() {
        "anthropic" => PackageKind::Anthropic,
        "openai" => PackageKind::OpenAI,
        "ollama" => PackageKind::Ollama,
        "openrouter" => PackageKind::OpenRouter,
        _ => PackageKind::OpenAI, // default to OpenAI-compatible
    };
...
pub extern "C" fn forge_ai_ask_with_image(
    ...
    let messages = match config.package {
        PackageKind::Anthropic => {
            vec![Message {
                role: "user".to_string(),
                content: json!([
...
// ---------------------------------------------------------------------------
// Channel-based streaming (cross-package linking with std-channel)
// ---------------------------------------------------------------------------
...
pub extern "C" fn forge_ai_stream_to_channel(
    ...
    let (ch_create, ch_send, ch_close) = match resolve_channel_fns() {
        Some(fns) => fns,
        None => return -1, // channel package not linked
    };
