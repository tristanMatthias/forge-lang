...
/// e.g., `cors: bool = false` or `@default description: string = ""`
#[derive(Debug, Clone)]
...
/// Component template definition from package.fg
/// e.g., `component model($name, schema) { ... }`
#[derive(Debug, Clone)]
...
pub enum ComponentTemplateItem {
    /// `type $name = $schema` — generate TypeDecl from user's schema
    /// If `visible_only` is true, fields with @hidden annotation are excluded
    TypeFromSchema { visible_only: bool },
    /// `on startup { ... }` — lifecycle statements (contain $* placeholders)
    ...
    InitFn(Vec<Statement>),
    /// `fn drop() { ... }` — cleanup, runs when instance goes out of scope
    DropFn(Vec<Statement>),
    /// `let field: type` — instance state (becomes a struct field)
    FieldDecl {
        name: String,
        type_ann: TypeExpr,
        span: Span,
    },
    /// Function template: `fn $name.method(...) { ... }`
    FnTemplate {
        method_name: String,
        decl: Statement,
    },
    ...
    ExternFn(Statement),
    /// `on after_children { ... }` — startup stmts emitted after nested component expansion
    OnAfterChildren(Vec<Statement>),
