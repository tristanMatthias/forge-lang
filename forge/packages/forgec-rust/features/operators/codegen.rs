...
impl<'ctx> Codegen<'ctx> {
    pub(crate) fn compile_binary_op(
        ...
        let is_null_compare =
            matches!(right_expr, Expr::NullLit(_)) || matches!(left_expr, Expr::NullLit(_));
        if is_null_compare && matches!(op, BinaryOp::Eq | BinaryOp::NotEq) {
            ...
            if nullable_type.is_nullable() && nullable_val.is_struct_value() {
                ...
                return Some(
                    self.builder
                        .build_int_z_extend(cmp, self.context.i8_type(), "null_cmp_ext")
                        .unwrap()
                        .into(),
                );
            }
        ...
        if lhs_is_ptr || rhs_is_ptr {
            match op {
                ...
                BinaryOp::Eq | BinaryOp::NotEq => {
                    ...
                    let (ptr_side, null_side) = if lhs_is_ptr { (lhs, rhs) } else { (rhs, lhs) };
            ...
            || rhs.is_float_value()
        {
            ...
            return match op {
                ...
                BinaryOp::Mod => Some(
                    self.builder
                        .build_float_rem(lhs_f, rhs_f, "fmod")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Eq => {
                    ...
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "feq_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::NotEq => {
                    let cmp = self
                        .builder
                        .build_float_compare(inkwell::FloatPredicate::ONE, lhs_f, rhs_f, "fne")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "fne_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::Lt => {
                    let cmp = self
                        .builder
                        .build_float_compare(inkwell::FloatPredicate::OLT, lhs_f, rhs_f, "flt")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "flt_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::LtEq => {
                    let cmp = self
                        .builder
                        .build_float_compare(inkwell::FloatPredicate::OLE, lhs_f, rhs_f, "fle")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "fle_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::Gt => {
                    let cmp = self
                        .builder
                        .build_float_compare(inkwell::FloatPredicate::OGT, lhs_f, rhs_f, "fgt")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "fgt_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::GtEq => {
                    let cmp = self
                        .builder
                        .build_float_compare(inkwell::FloatPredicate::OGE, lhs_f, rhs_f, "fge")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "fge_ext")
                            .unwrap()
                            .into(),
                    )
                }
        ...
        if lhs.is_int_value() && rhs.is_int_value() {
            ...
            return match op {
                BinaryOp::Add => Some(
                    self.builder
                        .build_int_add(lhs_i, rhs_i, "add")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Sub => Some(
                    self.builder
                        .build_int_sub(lhs_i, rhs_i, "sub")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Mul => Some(
                    self.builder
                        .build_int_mul(lhs_i, rhs_i, "mul")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Div => Some(
                    self.builder
                        .build_int_signed_div(lhs_i, rhs_i, "div")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Mod => Some(
                    self.builder
                        .build_int_signed_rem(lhs_i, rhs_i, "mod")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Eq => {
                    let cmp = self
                        .builder
                        .build_int_compare(IntPredicate::EQ, lhs_i, rhs_i, "eq")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "eq_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::NotEq => {
                    let cmp = self
                        .builder
                        .build_int_compare(IntPredicate::NE, lhs_i, rhs_i, "ne")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "ne_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::Lt => {
                    let cmp = self
                        .builder
                        .build_int_compare(IntPredicate::SLT, lhs_i, rhs_i, "lt")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "lt_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::LtEq => {
                    let cmp = self
                        .builder
                        .build_int_compare(IntPredicate::SLE, lhs_i, rhs_i, "le")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "le_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::Gt => {
                    let cmp = self
                        .builder
                        .build_int_compare(IntPredicate::SGT, lhs_i, rhs_i, "gt")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "gt_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::GtEq => {
                    let cmp = self
                        .builder
                        .build_int_compare(IntPredicate::SGE, lhs_i, rhs_i, "ge")
                        .unwrap();
                    Some(
                        self.builder
                            .build_int_z_extend(cmp, self.context.i8_type(), "ge_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::And => {
                    ...
                    Some(
                        self.builder
                            .build_int_z_extend(result, self.context.i8_type(), "and_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::Or => {
                    ...
                    Some(
                        self.builder
                            .build_int_z_extend(result, self.context.i8_type(), "or_ext")
                            .unwrap()
                            .into(),
                    )
                }
                BinaryOp::BitAnd => Some(
                    self.builder
                        .build_and(lhs_i, rhs_i, "bitand")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::BitOr => {
                    Some(self.builder.build_or(lhs_i, rhs_i, "bitor").unwrap().into())
                }
                BinaryOp::BitXor => Some(
                    self.builder
                        .build_xor(lhs_i, rhs_i, "bitxor")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Shl => Some(
                    self.builder
                        .build_left_shift(lhs_i, rhs_i, "shl")
                        .unwrap()
                        .into(),
                ),
                BinaryOp::Shr => Some(
                    self.builder
                        .build_right_shift(lhs_i, rhs_i, true, "shr")
                        .unwrap()
                        .into(),
                ),
            };
        ...
        if left_type == Type::String && right_type == Type::String {
            if matches!(op, BinaryOp::Eq | BinaryOp::NotEq) {
                let val =
                    self.call_runtime("forge_string_eq", &[lhs.into(), rhs.into()], "string_eq")?;
                if matches!(op, BinaryOp::NotEq) {
                    let int_v = val.into_int_value();
                    let zero = int_v.get_type().const_zero();
                    let negated = self
                        .builder
                        .build_int_compare(IntPredicate::EQ, int_v, zero, "string_neq")
                        .unwrap();
                    return Some(
                        self.builder
                            .build_int_z_extend(negated, self.context.i8_type(), "neq_ext")
                            .unwrap()
                            .into(),
                    );
                }
            ...
            if matches!(
                ...
                let cmp_val = self.call_runtime(
                    "forge_string_compare",
                    &[lhs.into(), rhs.into()],
                    "str_cmp",
                )?;
                ...
                let result = self
                    .builder
                    .build_int_compare(pred, cmp_i, zero, "str_ord")
                    .unwrap();
                return Some(
                    self.builder
                        .build_int_z_extend(result, self.context.i8_type(), "ord_ext")
                        .unwrap()
                        .into(),
                );
            }
                        ...
                        if matches!(op, BinaryOp::NotEq) {
                            if let Some(v) = val {
                                if v.is_int_value() {
                                    ...
                                    let negated = self
                                        .builder
                                        .build_int_compare(IntPredicate::EQ, int_v, zero, "not_eq")
                                        .unwrap();
                                    return Some(
                                        self.builder
                                            .build_int_z_extend(
