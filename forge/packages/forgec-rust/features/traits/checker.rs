            ...
            "ImplBlock" => {
                if let Some(data) = feature_data!(fe, ImplBlockData) {
                    ...
                    if let Some(trait_name) = &data.trait_name {
                        if let Some(required_methods) =
                            self.env.trait_methods.get(trait_name).cloned()
                        {
                            let impl_method_names: Vec<String> = data
                                .methods
                                .iter()
                                .filter_map(|m| match m {
                                    Statement::FnDecl { name, .. } => Some(name.clone()),
                                    Statement::Feature(fe) => {
                                        if let Some(fn_data) = feature_data!(
                                            fe,
                                            crate::features::functions::types::FnDeclData
                                        ) {
                            ...
                            if !missing.is_empty() {
                                ...
                                self.diagnostics.push(
                                    ...
                                    .with_help(format!(
                                        "add the missing method{} to satisfy the trait",
                                        if missing.len() == 1 { "" } else { "s" }
                                    )),
                                );
                            }
        ...
        match fe.kind {
            "TraitDecl" => {
                if let Some(data) = feature_data!(fe, TraitDeclData) {
                    ...
                    let required_methods: Vec<String> = data
                        .methods
                        .iter()
                    ...
                    self.env
                        .trait_methods
                        .insert(data.name.clone(), required_methods);
                    // Also store ALL method names (required + default)
                    let all_methods: Vec<String> =
                        data.methods.iter().map(|m| m.name.clone()).collect();
                    self.env
                        .trait_all_methods
                        .insert(data.name.clone(), all_methods);
                }
            }
            "ImplBlock" => {
                if let Some(data) = feature_data!(fe, ImplBlockData) {
                    ...
                    for m in &data.methods {
                        let (name, ret) = match m {
                            ...
                            Statement::Feature(fe) => {
                                if let Some(fn_data) =
                                    feature_data!(fe, crate::features::functions::types::FnDeclData)
                                {
                                    let ret = fn_data
                                        .return_type
                                        .as_ref()
