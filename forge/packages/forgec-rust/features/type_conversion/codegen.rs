...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn compile_int_conversion(
        &mut self,
        args: &[CallArg],
        ...
        match arg_type {
            ...
            Type::Bool => {
                ...
                let int_val = self
                    .builder
                    .build_int_z_extend(bool_val, self.context.i64_type(), "bool_to_int")
                    .unwrap();
                Some(int_val.into())
            }
            Type::String => self.call_runtime_expect(
                "forge_string_to_int",
                &[val.into()],
                "str_to_int",
    ...
    pub(crate) fn compile_float_conversion(
        &mut self,
        args: &[CallArg],
    ) -> Option<BasicValueEnum<'ctx>> {
        if args.is_empty() {
