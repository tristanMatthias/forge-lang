...
impl Parser {
    ...
    pub(crate) fn parse_component_block(&mut self, meta: &ComponentMeta) -> Option<Statement> {
        ...
        while !self.check(&TokenKind::LBrace) && !self.is_at_end() {
            match &self.peek()?.kind {
                ...
                TokenKind::Ident(name) => {
                    // Don't consume another component keyword as an arg
                    if self.registered_components.contains_key(name) && !args.is_empty() {
                        break;
                    }
                    let name = name.clone();
                    let span = self.advance()?.span;
                ...
                TokenKind::Newline => {
                    // After consuming at least one arg, newline ends this component declaration
                    if !args.is_empty() {
                        break;
                    }
                    self.advance();
        ...
        if let Some(ref default_key) = meta.default_config_key {
            if matches!(self.peek().map(|t| &t.kind), Some(TokenKind::StringLiteral(_))) {
                ...
                self.skip_newlines();
                // If no body follows, return with just the inline config
                if !self.check(&TokenKind::LBrace) {
                    return Some(Statement::ComponentBlock(ComponentBlockDecl {
                        component,
                        args,
        ...
        // Allow bare component declarations without body: `queue orders`
        if !self.check(&TokenKind::LBrace) {
            return Some(Statement::ComponentBlock(ComponentBlockDecl {
                component,
                args,
        ...
        while !self.check(&TokenKind::RBrace) {
            ...
            let is_block_keyword = match &self.peek()?.kind {
                TokenKind::Ident(n) if n == "under" || n == "middleware" => true,
                TokenKind::Ident(n) if self.registered_components.contains_key(n) => true,
                _ => false,
            };
            ...
            match &self.peek()?.kind {
                ...
                TokenKind::Ident(ref name) if self.registered_components.contains_key(name) => {
                    // Nested component block (e.g., flag, arg, option inside a command)
                    let nested_meta = self.registered_components.get(name).cloned().unwrap();
                    if let Some(stmt) = self.parse_component_block(&nested_meta) {
                        blocks.push(stmt);
                    }
                }
                TokenKind::Ident(_) => {
                ...
                TokenKind::StringLiteral(_) if meta.default_config_key.is_some() => {
                    // Bare string literal → assign to default config key
                    let span = self.peek()?.span;
                    if let Some(value) = self.parse_expr() {
                        config.push(ComponentConfig {
                            key: meta.default_config_key.as_ref().unwrap().clone(),
                            value,
                            span,
                        });
                    }
    ...
    pub(crate) fn parse_component_template_def(&mut self) -> Option<Statement> {
        ...
        // Parse params: ($name, for $ref, schema)
        ...
        loop {
            ...
            match &self.peek()?.kind {
                TokenKind::For => {
                    ...
                    self.skip_newlines();
                    let _ref_name = self.expect_ident()?; // $ref
                    has_ref = true;
                }
                TokenKind::Ident(name) => {
                    if name == "schema" {
            ...
            match &self.peek()?.kind {
                ...
                TokenKind::Ident(name) if name == "config" => {
                    ...
                    while !self.check(&TokenKind::RBrace) && !self.is_at_end() {
                        ...
                        // Parse annotations after the entry (e.g., `description: string = "" @default`)
                        let mut annotations = Vec::new();
                        while self.check(&TokenKind::At) {
                            if let Some(ann) = self.parse_component_annotation() {
                ...
                TokenKind::Type => {
                    // `type Name = Schema` or `type Name = SchemaVisible` → TypeFromSchema
                    self.advance(); // consume 'type'
                    self.skip_newlines();
                    let _type_name = self.expect_ident()?; // $name
                    self.skip_newlines();
                ...
                TokenKind::Fn => {
                    // fn init(), fn drop(), fn $name.method(...), or fn $ref.method(...)
                    ...
                    if first_name == "init" {
                        // fn init() { ... } — constructor
                        self.expect(&TokenKind::LParen)?;
                        let _params = self.parse_params()?; // ignored, init takes no explicit params
                        self.skip_newlines();
                        let body = self.parse_block()?;
                        items.push(ComponentTemplateItem::InitFn(body.statements));
                    } else if first_name == "drop" {
                        // fn drop() { ... } — cleanup
                        self.expect(&TokenKind::LParen)?;
                        let _params = self.parse_params()?;
                        self.skip_newlines();
                        let body = self.parse_block()?;
                        items.push(ComponentTemplateItem::DropFn(body.statements));
                    } else {
                        // Instance method: fn method(...) { ... }
                        // Automatically scoped to the component instance during expansion.
                        ...
                        let fn_name = format!("$fn_{}", method_name);
                ...
                TokenKind::Let => {
                    // let field: type — instance state (struct field)
                    let let_span = self.advance()?.span; // consume 'let'
                    self.skip_newlines();
                    let field_name = self.expect_ident()?;
                    self.expect(&TokenKind::Colon)?;
                    self.skip_newlines();
                    let type_ann = self.parse_type_expr()?;
                    items.push(ComponentTemplateItem::FieldDecl {
                        name: field_name,
                        type_ann,
                        span: let_span,
                    });
