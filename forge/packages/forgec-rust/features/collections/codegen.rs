...
impl<'ctx> Codegen<'ctx> {
    ...
    fn setup_list_iter(
        ...
        let idx_alloca = self
            .builder
            .build_alloca(self.context.i64_type(), &format!("{}_idx", prefix))
            .unwrap();
        self.builder
        ...
        let idx = self
            .builder
            .build_load(self.context.i64_type(), idx_alloca, "i")
            .unwrap()
            .into_int_value();
        let cond = self
            .builder
            .build_int_compare(IntPredicate::SLT, idx, list_len, "cond")
            .unwrap();
        self.builder
        ...
        let elem_ptr = unsafe {
            self.builder
                .build_gep(elem_llvm_ty, data_ptr, &[idx], "ep")
                .unwrap()
        };
        let elem_val = self
            .builder
    ...
    fn setup_search_loop(
        ...
        let cond = self
            .builder
            .build_int_compare(IntPredicate::SLT, idx, len, "cond")
            .unwrap();
        self.builder
            .build_conditional_branch(cond, body_bb, end_bb)
            .unwrap();
    ...
    fn finish_list_iter(&self, setup: &ListIterSetup<'ctx>) {
        ...
        self.builder
            .build_unconditional_branch(setup.loop_bb)
            .unwrap();
    ...
    fn setup_predicate_iter(
        ...
        let next_bb = self
            .context
            .append_basic_block(function, &format!("{}_next", prefix));
        ...
        if match_on_true {
            self.builder
                .build_conditional_branch(pred_bool, on_match_bb, next_bb)
                .unwrap();
        } else {
            self.builder
                .build_conditional_branch(pred_bool, next_bb, on_match_bb)
                .unwrap();
        }
    ...
    pub(crate) fn compile_list_lit(&mut self, elements: &[Expr]) -> Option<BasicValueEnum<'ctx>> {
        ...
        let data_ptr = self
            .call_runtime("forge_alloc", &[total_size.into()], "list_data")?
            .into_pointer_value();
        ...
        for (i, val) in elem_vals.iter().enumerate() {
            ...
            let elem_ptr = unsafe {
                self.builder
                    .build_gep(elem_llvm_ty, data_ptr, &[idx], &format!("elem_{}_ptr", i))
                    .unwrap()
            };
    ...
    pub(crate) fn compile_map_lit(
        ...
        if entries.is_empty() {
            ...
            let keys_ptr = self
                .call_runtime("forge_alloc", &[one_byte.into()], "map_keys")?
                .into_pointer_value();
            let vals_ptr = self
                .call_runtime("forge_alloc", &[one_byte.into()], "map_vals")?
                .into_pointer_value();
            let map_type = self
                .context
                .struct_type(&[ptr_type.into(), ptr_type.into(), i64_type.into()], false);
            let mut map_val = map_type.get_undef();
            map_val = self
                .builder
                .build_insert_value(map_val, keys_ptr, 0, "mk")
                .unwrap()
                .into_struct_value();
            map_val = self
                .builder
                .build_insert_value(map_val, vals_ptr, 1, "mv")
                .unwrap()
                .into_struct_value();
            map_val = self
                .builder
                .build_insert_value(map_val, i64_type.const_zero(), 2, "ml")
                .unwrap()
                .into_struct_value();
        ...
        let keys_total = self
            ...
            .unwrap();
        let keys_ptr = self
            .call_runtime("forge_alloc", &[keys_total.into()], "keys_ptr")?
            .into_pointer_value();
        ...
        let vals_total = self
            ...
            .unwrap();
        let vals_ptr = self
            .call_runtime("forge_alloc", &[vals_total.into()], "vals_ptr")?
            .into_pointer_value();
        ...
        for (i, (key_expr, val_expr)) in entries.iter().enumerate() {
            ...
            let kp = unsafe {
                self.builder
                    .build_gep(key_llvm_ty, keys_ptr, &[idx], "kp")
                    .unwrap()
            };
            ...
            let vp = unsafe {
                self.builder
                    .build_gep(val_llvm_ty, vals_ptr, &[idx], "vp")
                    .unwrap()
            };
        ...
        map_val = self
            .builder
            .build_insert_value(map_val, keys_ptr, 0, "mp_keys")
            .unwrap()
            .into_struct_value();
        map_val = self
            .builder
            .build_insert_value(map_val, vals_ptr, 1, "mp_vals")
            .unwrap()
            .into_struct_value();
        map_val = self
            .builder
    ...
    pub(crate) fn compile_list_lit_feature(
        ...
        feature_codegen!(self, fe, ListLitData, |data| self
            .compile_list_lit(&data.elements))
    }
    ...
    pub(crate) fn compile_map_lit_feature(
        ...
        feature_codegen!(self, fe, MapLitData, |data| self
            .compile_map_lit(&data.entries))
    }
    ...
    pub(crate) fn compile_list_pop(
        ...
        let result_alloca = self
            .builder
            .build_alloca(nullable_llvm_ty, "pop_result")
            .unwrap();
        self.builder
            .build_store(
                result_alloca,
                nullable_llvm_ty.into_struct_type().const_zero(),
            )
            .unwrap();
        ...
        let last_ptr = unsafe {
            self.builder
                .build_gep(elem_llvm_ty, data_ptr, &[new_len], "last_ptr")
                .unwrap()
        };
        let last_val = self
            .builder
            .build_load(elem_llvm_ty, last_ptr, "last_val")
            .unwrap();
        ...
        let result = self
            .builder
            .build_load(nullable_llvm_ty, result_alloca, "pop_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_filter(
        ...
        let total = self
            .builder
            .build_int_mul(elem_size, list_len, "total")
            .unwrap();
        let result_ptr = self
            .call_runtime("forge_alloc", &[total.into()], "filter_buf")?
            .into_pointer_value();

        // Result count alloca
        let count_alloca = self
            .builder
            .build_alloca(self.context.i64_type(), "filter_count")
            .unwrap();
        self.builder
            .build_store(count_alloca, self.context.i64_type().const_zero())
            .unwrap();
        ...
        let count = self
            .builder
            .build_load(self.context.i64_type(), count_alloca, "c")
            .unwrap()
            .into_int_value();
        let dest_ptr = unsafe {
            self.builder
                .build_gep(elem_llvm_ty, result_ptr, &[count], "dp")
                .unwrap()
        };
        ...
        let final_count = self
            .builder
            .build_load(self.context.i64_type(), count_alloca, "fc")
            .unwrap();
    ...
    pub(crate) fn compile_list_map(
        ...
        let total = self
            .builder
            .build_int_mul(out_size, list_len, "total")
            .unwrap();
        let result_ptr = self
            .call_runtime("forge_alloc", &[total.into()], "map_buf")?
            .into_pointer_value();
        ...
        let dest_ptr = unsafe {
            self.builder
                .build_gep(out_llvm_ty, result_ptr, &[setup.idx], "dp")
                .unwrap()
        };
    ...
    pub(crate) fn compile_list_sum(
        ...
        let sum_alloca = self
            .builder
            .build_alloca(self.context.i64_type(), "sum")
            .unwrap();
        self.builder
            .build_store(sum_alloca, self.context.i64_type().const_zero())
            .unwrap();
        ...
        let acc = self
            .builder
            .build_load(self.context.i64_type(), sum_alloca, "acc")
            .unwrap()
            .into_int_value();
        ...
        let result = self
            .builder
            .build_load(self.context.i64_type(), sum_alloca, "sum_result")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_find(
        ...
        let result_alloca = self
            .builder
            .build_alloca(nullable_llvm_ty, "find_result")
            .unwrap();
        self.builder
            .build_store(
                result_alloca,
        ...
        self.builder.build_store(result_alloca, wrapped).unwrap();
        self.builder
            .build_unconditional_branch(setup.end_bb)
            .unwrap();

        self.builder.position_at_end(setup.end_bb);
        let result = self
            .builder
            .build_load(nullable_llvm_ty, result_alloca, "find_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_any(
        ...
        let result_alloca = self
            .builder
            .build_alloca(self.context.i8_type(), "any_result")
            .unwrap();
        self.builder
            .build_store(result_alloca, self.context.i8_type().const_zero())
            .unwrap();

        let (setup, _found_bb, _next_bb) =
            self.setup_predicate_iter(list_val, elem_type, closure_arg, "any", "any_found", true)?;

        // Found: set result to true and exit loop
        self.builder
            .build_store(result_alloca, self.context.i8_type().const_int(1, false))
            .unwrap();
        self.builder
            .build_unconditional_branch(setup.end_bb)
            .unwrap();
        ...
        let result = self
            .builder
            .build_load(self.context.i8_type(), result_alloca, "any_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_all(
        ...
        let result_alloca = self
            .builder
            .build_alloca(self.context.i8_type(), "all_result")
            .unwrap();
        self.builder
            .build_store(result_alloca, self.context.i8_type().const_int(1, false))
            .unwrap();

        let (setup, _fail_bb, _next_bb) =
            self.setup_predicate_iter(list_val, elem_type, closure_arg, "all", "all_fail", false)?;
        ...
        let result = self
            .builder
            .build_load(self.context.i8_type(), result_alloca, "all_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_enumerate(
        ...
        let total = self
            .builder
            .build_int_mul(tuple_size, list_len, "total")
            .unwrap();
        let result_ptr = self
            .call_runtime("forge_alloc", &[total.into()], "enum_buf")?
            .into_pointer_value();
        ...
        tuple_val = self
            .builder
            .build_insert_value(tuple_val, setup.idx, 0, "t0")
            .unwrap()
            .into_struct_value();
        tuple_val = self
            .builder
            .build_insert_value(tuple_val, setup.elem_val, 1, "t1")
            .unwrap()
            .into_struct_value();

        let dest_ptr = unsafe {
            self.builder
                .build_gep(tuple_llvm_ty, result_ptr, &[setup.idx], "dp")
                .unwrap()
        };
    ...
    pub(crate) fn compile_list_join(
        ...
        let result_alloca = self
            .builder
            .build_alloca(string_llvm_ty, "join_result")
            .unwrap();
        let empty_str = self.build_string_literal("");
        self.builder.build_store(result_alloca, empty_str).unwrap();

        let (loop_bb, _body_bb, end_bb, idx_alloca, idx) = self.setup_search_loop(list_len, "join");

        let elem_ptr = unsafe {
            self.builder
                .build_gep(string_llvm_ty, data_ptr, &[idx], "ep")
                .unwrap()
        };
        let elem_val = self
            .builder
            .build_load(string_basic_ty, elem_ptr, "elem")
            .unwrap();
        ...
        let current = self
            .builder
            .build_load(string_basic_ty, result_alloca, "cur")
            .unwrap();
        ...
        self.builder
            .build_conditional_branch(is_first, nosep_block, sep_block)
            .unwrap();
        ...
        let result = self
            .builder
            .build_load(string_basic_ty, result_alloca, "join_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_reduce(
        ...
        let acc_val = self
            .builder
            .build_load(init_val.get_type(), acc_alloca, "acc")
            .unwrap();
        let new_acc = self.compile_closure_inline_2(
            closure_arg,
            acc_val,
            &acc_type,
            setup.elem_val,
        ...
        let result = self
            .builder
            .build_load(init_val.get_type(), acc_alloca, "reduce_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_list_sorted(
        ...
        // Clone the list data
        let new_ptr = self
            .call_runtime("forge_alloc", &[total.into()], "sort_buf")?
            .into_pointer_value();
        self.builder
            .build_memcpy(new_ptr, 1, data_ptr, 1, total)
            .ok();
        ...
        let sort_fn = self
            .module
            .get_function("forge_list_sort_int")
            .unwrap_or_else(|| {
                let ft = self
                    .context
                    .void_type()
        ...
        self.builder
            .build_call(sort_fn, &[new_ptr.into(), list_len.into()], "")
            .unwrap();
    ...
    pub(crate) fn compile_map_has(
        ...
        let result_alloca = self
            .builder
            .build_alloca(self.context.i8_type(), "has_result")
            .unwrap();
        self.builder
            .build_store(result_alloca, self.context.i8_type().const_zero())
            .unwrap();

        let (loop_bb, _body_bb, end_bb, idx_alloca, idx) = self.setup_search_loop(map_len, "has");
        ...
        let result = self
            .builder
            .build_load(self.context.i8_type(), result_alloca, "has_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_map_get(
        ...
        let result_alloca = self
            .builder
            .build_alloca(nullable_llvm_ty, "get_result")
            .unwrap();
        self.builder
            .build_store(
                result_alloca,
        ...
        let (loop_bb, _body_bb, end_bb, idx_alloca, idx) = self.setup_search_loop(map_len, "get");
        ...
        let result = self
            .builder
            .build_load(nullable_llvm_ty, result_alloca, "get_val")
            .unwrap();
        Some(result)
    }
    ...
    pub(crate) fn compile_map_index_assign(&mut self, object: &Expr, index: &Expr, value: &Expr) {
        ...
        let new_key = match self.compile_expr(index) {
            Some(v) => v,
            None => return,
        };
        let new_val = match self.compile_expr(value) {
            Some(v) => v,
            None => return,
        };
        ...
        let map_val = self
            .builder
            .build_load(map_llvm_ty, var_ptr, "map_cur")
            .unwrap()
            .into_struct_value();
        let keys_ptr = self
            .builder
            .build_extract_value(map_val, 0, "keys")
            .unwrap()
            .into_pointer_value();
        let vals_ptr = self
            .builder
            .build_extract_value(map_val, 1, "vals")
            .unwrap()
            .into_pointer_value();
        let map_len = self
            .builder
            .build_extract_value(map_val, 2, "len")
            .unwrap()
            .into_int_value();
        ...
        let idx_alloca = self
            .builder
            .build_alloca(self.context.i64_type(), "ms_idx")
            .unwrap();
        self.builder
            .build_store(idx_alloca, self.context.i64_type().const_zero())
            .unwrap();
        ...
        let cond = self
            .builder
            .build_int_compare(IntPredicate::SLT, idx, map_len, "cond")
            .unwrap();
        self.builder
        ...
        let found_idx = self
            .builder
            .build_load(self.context.i64_type(), idx_alloca, "fi")
            .unwrap()
            .into_int_value();
        let vp = unsafe {
            self.builder
                .build_gep(val_llvm_ty, vals_ptr, &[found_idx], "vp")
                .unwrap()
        };
        ...
        let key_size = key_llvm_ty.size_of().unwrap();
        let new_keys_total = self
            .builder
            .build_int_mul(key_size, new_len, "nkt")
            .unwrap();
        let new_keys_ptr = self
            .call_runtime("forge_alloc", &[new_keys_total.into()], "new_keys")
            .unwrap()
            .into_pointer_value();

        // Allocate new vals array
        let val_size = val_llvm_ty.size_of().unwrap();
        let new_vals_total = self
            .builder
            .build_int_mul(val_size, new_len, "nvt")
            .unwrap();
        let new_vals_ptr = self
            .call_runtime("forge_alloc", &[new_vals_total.into()], "new_vals")
            .unwrap()
            .into_pointer_value();
        ...
        self.builder
            .build_conditional_branch(has_old, copy_bb, append_bb)
            .unwrap();

        self.builder.position_at_end(copy_bb);
        let old_keys_total = self
            .builder
            .build_int_mul(key_size, map_len, "okt")
            .unwrap();
        self.builder
            .build_memcpy(new_keys_ptr, 1, keys_ptr, 1, old_keys_total)
            .ok();
        let old_vals_total = self
            .builder
            .build_int_mul(val_size, map_len, "ovt")
            .unwrap();
        self.builder
            .build_memcpy(new_vals_ptr, 1, vals_ptr, 1, old_vals_total)
            .ok();
        ...
        let new_key_ptr = unsafe {
            self.builder
                .build_gep(key_llvm_ty, new_keys_ptr, &[map_len], "nkp")
                .unwrap()
        };
        self.builder.build_store(new_key_ptr, new_key).unwrap();
        let new_val_ptr = unsafe {
            self.builder
                .build_gep(val_llvm_ty, new_vals_ptr, &[map_len], "nvp")
                .unwrap()
        };
        ...
        updated = self
            .builder
            .build_insert_value(updated, new_keys_ptr, 0, "uk")
            .unwrap()
            .into_struct_value();
        updated = self
            .builder
            .build_insert_value(updated, new_vals_ptr, 1, "uv")
            .unwrap()
            .into_struct_value();
        updated = self
            .builder
            .build_insert_value(updated, new_len, 2, "ul")
            .unwrap()
            .into_struct_value();
    ...
    pub(crate) fn compile_key_eq(
        ...
        match key_type {
            Type::String => {
                let val = self
                    .call_runtime("forge_string_eq", &[a.into(), b.into()], "str_eq")
                    .unwrap()
                    .into_int_value();
                self.builder
                    .build_int_compare(
