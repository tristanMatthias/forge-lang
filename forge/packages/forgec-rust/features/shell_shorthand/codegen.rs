...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn compile_dollar_exec_feature(
        ...
        feature_codegen!(self, fe, DollarExecData, |data| self
            .compile_dollar_exec(&data.parts))
    }
    ...
    pub(crate) fn compile_dollar_exec(
        ...
        let cmd_ptr = self
            .builder
            .build_extract_value(cmd_str.into_struct_value(), 0, "cmd_ptr")
            .unwrap();
        ...
        let exec_fn = self
            .module
            .get_function("forge_process_exec")
            .unwrap_or_else(|| {
        ...
        let result = self
            .builder
            .build_call(exec_fn, &[cmd_ptr.into()], "exec_result")
            .unwrap();
        ...
        let len = self
            ...
            .unwrap();
        let stdout_str = self.call_runtime(
            "forge_string_new",
