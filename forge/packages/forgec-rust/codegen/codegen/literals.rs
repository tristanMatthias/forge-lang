...
impl<'ctx> Codegen<'ctx> {
    ...
    pub(crate) fn compile_enum_constructor(
        ...
        let enum_alloca = self
            .builder
            .build_alloca(enum_struct_type, "enum_tmp")
            .unwrap();
        self.builder
            .build_store(enum_alloca, enum_struct_type.const_zero())
            .unwrap();

        // Set tag at index 0
        let tag_ptr = self
            .builder
            .build_struct_gep(enum_struct_type, enum_alloca, 0, "tag_ptr")
            .unwrap();
        self.builder
            .build_store(
                tag_ptr,
        ...
        if !args.is_empty() && !variant.fields.is_empty() {
            let payload_ptr = self
                .builder
                .build_struct_gep(enum_struct_type, enum_alloca, 1, "payload_ptr")
                .unwrap();

            // Build a struct type for this variant's fields
            // Boxed fields are stored as i64 (pointer-sized) instead of the recursive type
            let variant_field_types: Vec<inkwell::types::BasicTypeEnum<'ctx>> = variant
                .fields
                .iter()
                .enumerate()
                .map(|(i, (_, ty))| {
            ...
            for (i, arg) in args.iter().take(valid_arg_count).enumerate() {
                if let Some(val) = self.compile_expr(&arg.value) {
                    let stored_val = if variant.boxed_fields.contains(&i) {
                        ...
                        let heap_ptr = self
                            .call_runtime("forge_alloc", &[val_size.into()], "boxed_field")
                            .unwrap()
                            .into_pointer_value();
                        self.builder.build_store(heap_ptr, val).unwrap();
                        // Convert pointer to i64 for storage in the i64-slot union
                        self.builder
                            .build_ptr_to_int(heap_ptr, self.context.i64_type(), "boxed_ptr_int")
                            .unwrap()
                            .into()
                    } else {
                        val
        ...
        let result = self
            .builder
            .build_load(enum_struct_type, enum_alloca, "enum_val")
            .unwrap();
        Some(result)
    }
}
