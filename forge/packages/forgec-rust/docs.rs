...
// ── Unified features (shared with forge lang) ──────────────────────

/// Show a compact symbols listing of all documented project symbols.
pub fn show_symbols(docs: &ProjectDocs) {
    println!();
    ...
    if !docs.types.is_empty() {
        println!("  {}", bold("Types"));
        for t in &docs.types {
            let kind_name = format!("{} {}", t.kind, t.name);
            let loc = format!("{}:{}", t.file, t.line);
            println!("    {:<44} {}", kind_name, dim(&loc));
        }
        println!();
    }
    ...
    if !docs.enums.is_empty() {
        println!("  {}", bold("Enums"));
        for e in &docs.enums {
            let name = format!("enum {}", e.name);
            let loc = format!("{}:{}", e.file, e.line);
            println!("    {:<44} {}", name, dim(&loc));
        }
        println!();
    }
    ...
    if !docs.constants.is_empty() {
        ...
        for c in &docs.constants {
            let sig = match &c.type_ann {
                Some(t) => format!("{} {}: {}", c.kind, c.name, t),
                None => format!("{} {}", c.kind, c.name),
            };
            let loc = format!("{}:{}", c.file, c.line);
            println!("    {:<44} {}", sig, dim(&loc));
        }
        println!();
    }
...
pub fn show_short(query: &str, docs: &ProjectDocs) -> bool {
    let query_lower = query.to_lowercase();
...
/// Fuzzy/substring search across all project documentation.
pub fn show_search(query: &str, docs: &ProjectDocs) {
    let query_lower = query.to_lowercase();
    ...
    for f in &docs.functions {
        ...
        if score > 0 {
            results.push(SearchResult {
                category: "Functions",
                name: f.name.clone(),
                description: first_line_or_empty(&f.doc),
                score,
            });
        }
    }
    ...
    for t in &docs.types {
        ...
        let doc_lower = t.doc.to_lowercase();
        let score = if name_lower == query_lower {
            5
        } else if name_lower.starts_with(&query_lower) {
            3
        ...
        if score > 0 {
            results.push(SearchResult {
                category: "Types",
                name: t.name.clone(),
                description: first_line_or_empty(&t.doc),
                score,
            });
        }
    }
    ...
    for e in &docs.enums {
        ...
        let doc_lower = e.doc.to_lowercase();
        let variants_str = e.variants.join(", ").to_lowercase();
        let score = if name_lower == query_lower {
            5
        } else if name_lower.starts_with(&query_lower) {
            3
        } else if name_lower.contains(&query_lower) || variants_str.contains(&query_lower) {
            2
        } else if doc_lower.contains(&query_lower) {
            1
        } else {
        ...
        if score > 0 {
            results.push(SearchResult {
                category: "Enums",
                name: e.name.clone(),
                description: first_line_or_empty(&e.doc),
                score,
            });
        }
    }
    ...
    for c in &docs.constants {
        ...
        let doc_lower = c.doc.to_lowercase();
        let score = if name_lower == query_lower {
            5
        } else if name_lower.starts_with(&query_lower) {
            3
        ...
        if score > 0 {
            results.push(SearchResult {
                category: "Constants",
                name: c.name.clone(),
                description: first_line_or_empty(&c.doc),
                score,
            });
        }
    }
    ...
    if results.is_empty() {
        ...
        println!("  Try: {}", cyan("compiler docs"));
        println!();
        return;
    }
    ...
    let categories = ["Functions", "Types", "Enums", "Constants"];
    ...
    println!("  {}", dim("Hint: forge docs <name> for full docs"));
    println!();
}

/// Generate LLM-friendly project documentation.
pub fn show_llm(docs: &ProjectDocs) {
    ...
    if !docs.functions.is_empty() {
        ...
        for f in &docs.functions {
            ...
            println!("{}{}", sig, export);
            if !f.doc.is_empty() {
                println!("# {}", f.doc.lines().next().unwrap_or(""));
            }
        }
        println!();
    }
    ...
    if !docs.types.is_empty() {
        ...
        for t in &docs.types {
            ...
            if !t.doc.is_empty() {
                println!("# {}", t.doc.lines().next().unwrap_or(""));
            }
        }
        println!();
    }
    ...
    if !docs.enums.is_empty() {
        ...
        for e in &docs.enums {
            ...
            println!("enum {} {{ {} }}{}", e.name, e.variants.join(", "), export);
            if !e.doc.is_empty() {
                println!("# {}", e.doc.lines().next().unwrap_or(""));
            }
        }
        println!();
    }
    ...
    if !docs.constants.is_empty() {
        ...
        for c in &docs.constants {
            ...
            if type_str.is_empty() {
                ...
                println!("{} {}: {}", c.kind, c.name, type_str);
            }
        }
        println!();
    }
