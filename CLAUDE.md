...
**A killed run reaps its own orphans now (t-ce5t).** `--sweep` and `--guarded` register the run, so its work is reaped on EVERY death: traps handle normal/INT/TERM exits, a nanny process covers SIGKILL (which no trap can), and the next run clears anything left when a whole group died at once. `--guarded`'s watchdog also stops when its owner dies, instead of polling forever ready to SIGKILL the next run's compiles.

Two scoping rules are load-bearing, both learned the hard way on real runs. **The unit is the orphaned TREE, not the process**: a killed sweep's test-unit binary is not named `bs2`, so a name-based kill (the memguard's included) leaves it alive and it keeps spawning fresh multi-GB compiles — each with a live parent, so they do not even look like strays. **And never the whole process group**: a group is routinely shared (the Claude Code bash tool keeps ONE persistent shell, so every command you run shares one), so group-wide killing takes out siblings — in the caught case, the `tail` of the invoking command's own pipeline, which is why the run reported a bare "Terminated". A run nested inside another heavy run downgrades to descendant-only, which is why a spec test can drive this tooling without endangering the suite around it. Guards: `test_runner/tests/stray_reap_test.av` — 11 specs that really SIGKILL a run mid-flight; mutation-verified (disabling the nanny turns the headline spec red).

**On a ≤16GB Claude-Code-Web container, `make test` doesn't just OOM — it takes the whole container down** (three restarts in one 2026-07-12 session, all mid-suite). The pattern that survives is now a first-class mode: **`make sweep`** (= `diagnose.sh --sweep`) — one sequential `bs2 test <dir>` per test directory (the dir list is derived from `bs2 test --list`, so tests outside `tests/` dirs are covered too), per-dir logs + `.ok` resume markers under `build/sweep/` (a kill resumes at the failing dir instead of restarting), loud failure at the first red dir (no later dir masks an earlier one), and a suite-wide tally on green. Resume markers are keyed on the suite-input hash (`--test-input-hash`: bs2 + every .av + runtime + seed + diagnose.sh + Makefile), so ANY input change drops them automatically — `FRESH=1` (or `--fresh`) still forces a re-run but is no longer needed for correctness (t-8rsg). A green FULL sweep (no dir args) writes `build/.tests_verified`, the pre-commit hook's suite-cache marker, so the hook skips the duplicate suite run at commit time. Extra args scope the sweep to specific dirs (scoped sweeps never write the marker). This is the sanctioned full-suite runner on a small box; pair it with `make selfhost` for the full `make test` equivalent.
...
- **For iterating on tests, use single-file runs** (`./build/bs2 test tests/<one>_test.av`), not a whole-dir fan-out. `bs2 test <dir>` fans out parallel shards — a memory spike. Set `AVRA_JOBS=1` (or `2`) if you must run a dir, and guard it.
...
**Sweep-first COMMITS on a small box.** The pre-commit hook's parallel per-file suite OOMs when it runs COLD — and any compiler-source edit (comments included) changes source fingerprints, so every shard compile goes cold. The commit pattern that survives: `make build-quick && make sweep` first (sequential, restart-resumable; FRESH=1 is no longer needed — markers self-freshen on any input change), THEN `git commit`. Since t-8rsg the green full sweep writes the hook's `.tests_verified` marker, so the hook SKIPS its duplicate suite pass entirely — the commit gate after a green sweep is just the selfhost fixed-point check (~30s), not a second suite run. Doc/test-only commits (no compiler source) skip the sweep. Also check `du -sh /tmp/build` when disk runs low: fixtures compiling with cwd=/tmp leak a compile cache; `make cache-gc` now reaches it (t-2qn0 — `--prune-tmp-scratch` sweeps `/tmp/avra_*` + `/tmp/build`; the residual cwd=/tmp compile foot-gun is t-wrgc).
