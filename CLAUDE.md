...
- **File-level consts COLLIDE across batched test files (t-5vze):** when several `*_test.av` files assemble into one shard binary, a top-level `const` whose name matches a sibling batch member's silently resolves to the SIBLING's value (first-match-wins, no duplicate error — unlike fns). Symptom: a fixture passes standalone but fails in `bs2 test <dir>` with another test's data (witness: two tests both naming `CONSUMER_SRC` — one compiled the other's consumer source). Until fixed, give test-file consts unique, file-prefixed names (`WALKER_CONSUMER_SRC`, not `CONSUMER_SRC`) — and suspect this class FIRST when a test flips between standalone and bundled runs.
