...
**A killed run reaps its own orphans now (t-ce5t).** `--sweep` and `--guarded` register the run, so its compiles are reaped on EVERY death: the traps handle normal/INT/TERM exits, a nanny process covers SIGKILL (which no trap can), and the next run clears anything left when a whole group died at once. `--guarded`'s watchdog also stops when its owner dies, instead of polling forever ready to SIGKILL the next run's compiles. Reaping is scoped by process group + recorded ancestry + a start-time guard, and is DOWNGRADED to descendant-only when the run is nested inside another heavy run — so a spec test driving this tooling can never kill the suite around it. Guards: `test_runner/tests/stray_reap_test.av` (it really SIGKILLs a run mid-flight; mutation-verified — disabling the nanny turns it red).
