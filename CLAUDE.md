...
- **The struct-shaped negative control is an OPEN finding — do not re-derive it, and do not assume the cause.** Measured with the local counter, 100 constructions each: a struct bound in a loop, a struct measured after its frame RETURNED, a struct never bound at all, a struct with no string field, and an INLINE struct literal all leave **exactly +100**, while a bare `List` local leaves 0. The last two rule out #1178's string-literal freshness denial (an inline literal is releasable by construction, `is_allocating_expr` admits it unconditionally). NOT attributed: `t_rc_frees` is bumped from `rc_set_remove`, which runs on an actual FREE, so a release that decrements a refcount without reaching zero never reaches the counter — **an over-RETAIN reads identically to a missing release here**, and telling them apart needs IR inspection, not another delta. Tracked as **t-0ks0 item 4**. (An earlier version of this note asserted the cause outright — "struct values get NO scope-exit release call site emitted at all" — which is too strong: #1181 showed a release call site IS emitted for a struct local whose tail never touches it.)
- **RESOLVED — it is TWO leaks plus a measurement artifact, so measure AFTER the allocating frame returns (t-0ks0 item 4).** A `+100` read from inside the still-live frame proves nothing: function-exit cleanup has not run yet. Measured after return, per 100 constructions: **inline struct literal in a loop → 99**, call-returned struct in a loop → **100**, call-returned with no string field → **100**, and ONE inline literal with no loop → **0**. (1) **The 99 is the tell** — exactly one release fires for a hundred constructions, so a loop-body local's release is emitted at FUNCTION exit and reclaims only the last value the alloca held: **N-1 leak per loop**. A single-local fn reads 0 because one release covers its one object, which is why `with_expr/tests/with_ptr_field_override_leak_test.av` passes and never contradicted this — it has no loop. (2) **A call-returned struct is never releasable**: `let d = mk()` needs `mk` in the return-ownership fresh set, and `init_is_owned` denies a `.String` field (#1178) *and* a `.ListLit` field (no `expr_is_fresh` arm, catch-all denies), so ordinary constructors aren't fresh and their results are never reclaimed — t-k6lm's leak-not-double-free bias doing what it says, and the larger of the two in practice. Also note a `List` is **not** a control for any of this: `avra_array_new` allocates via `avra_arr_xmalloc`, never `avra_rc_alloc`, so lists are invisible to this counter and `delta == 0` on a list scope measures nothing.
