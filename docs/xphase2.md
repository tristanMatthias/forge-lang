# Avra Compiler — Phase 1 Validation & Phase 2 Spec
...
Run each of the Phase 1 test programs (`hello.av` through `pipes.av`) and confirm they produce the expected output. If you skipped any, note which ones — we may need them as prereqs for Phase 2.
...
**JSON error output:**
```bash
avra build --error-format=json tests/programs/bad_type.av
# Should produce valid parseable JSON with error codes, spans, and suggestions
```

**LLVM IR output:**
```bash
avra build --emit-ir tests/programs/hello.av
...
Phase 2 adds the **module system**, **trait system**, **multi-file compilation**, **generics**, and the **project system** (avra.toml). At the end of Phase 2, Avra is a real multi-file language with a proper type system — ready for packages in Phase 3.

### Success Criteria

A multi-file project with traits, generics, the `use` keyword, and `avra.toml` compiles to a single binary. All test programs at the end of this document compile and run correctly.
...
The compiler scans the `src/` directory (or the directory specified in `avra.toml`). Each `.av` file is a module. Directory names are namespaces.

```
src/
  main.av           →  root module (entry point)
  math.av           →  module "math"
  utils/
    helpers.av      →  module "utils.helpers"
    format.av       →  module "utils.format"
  models/
    user.av         →  module "models.user"
```
...
error[E0030]: `validate_email` is private in module `models.user`
 ...
 2 | use models.user.validate_email
   |     ^^^^^^^^^^^^^^^^^^^^^^^^^^ private item
   |
   = help: mark it with `export` in models/user.av to make it public
```

#### 2.1.4 Compilation Pipeline (Multi-File)

```
1. Walk src/ directory, discover all .av files
...
### 2.4 The Project System (avra.toml)

#### 2.4.1 Minimal avra.toml for Phase 2
...
When `avra build` is run in a directory with `avra.toml`, it:
1. Reads the config
2. Discovers all `.av` files under the entry point's directory
3. Compiles them as a multi-file project
4. Outputs the binary to `./build/<project-name>`

When `avra build <file.av>` is run (no avra.toml), it compiles a single file as in Phase 1.

#### 2.4.2 avra.toml Parsing
...
entry = "src/main.av"              # default: "src/main.av"
...
These methods can be implemented in the C runtime (for Phase 2) or eventually in Avra itself (post-Phase 3 with self-hosting capabilities).
...
### 2.9.1 multi_module.av — Multi-file project
...
### 2.9.3 generics.av — User-defined generics
...
### 2.9.4 operators.av — Operator overloading
...
### 2.9.5 collections.av — Collection methods
...
### 2.9.6 visibility.av — Export/private enforcement
...
#### Step 1: avra.toml + Multi-File Discovery (Week 1)
- Parse avra.toml using `toml` + `serde`
- Walk source directory with `walkdir`
- Build module map (file path → module path)
- Modify driver to parse all files, not just one
- **Test: project with avra.toml compiles (even if single-file)**
...
- **Test: `multi_module.av` compiles and runs**
...
- **Test: `visibility.av` compiles, private access errors work**
...
- **Test: `traits.av` compiles and runs**
...
- **Test: `generics.av` compiles and runs**
...
- **Test: `operators.av` compiles and runs**
...
- **Test: `collections.av` compiles and runs**
...
- **Test: `drop.av` compiles and runs with correct drop order**
...
| E0060 | Invalid avra.toml configuration |
...
3. Multi-file projects with `avra.toml` work
