...
Cross-cutting subsystems the layers assume — deliberated and decided.

### A. Bootstrap of a near-total rewrite *(decided)*
*Tension:* self-hosting from a pinned seed means the **old** seed must build the
**new** compiler — but the new source uses constructs the old seed can't parse or
run. You can't cross the river in one jump.
*Decision:* the rewrite is **staged via the seed train** — the new compiler is
reached from the old seed by a chain of **buildable steps**, each advancing the
seed. All-touching changes (esp. the L1 node model) use a **compatibility shim /
dual representation** during migration (new node type + old-interface adapters),
so the tree **always builds**; passes migrate one at a time; the shim is deleted
last. **"Go hard" = aggressive *within* each buildable step, never a broken
tree** — the bootstrap floor is absolute. (So L1 is a *staged migration*, not one
atomic commit.)

### B. The query engine *(decided)*
*Tension:* "compiler = Salsa-like queries" needs a memoization + dependency-
tracking + invalidation engine; Salsa is a Rust library — we build our own.
*Decision:* **(1) a bounded runtime core** — the dependency graph + red-green /
early-cutoff invalidation (the L6 algorithm: last-changed vs last-verified stamps)
+ the content-addressed memo store (§4); **(2) a `@query` derive** (generated by
L4) that wraps each query fn with automatic dependency-recording + memoization.
The hard part is a *fixed* engine; per-query plumbing is *derived* (dogfoods L4).
Query **cycles error** by default with a clear diagnostic; legitimately-recursive
queries opt into fixpoint.

### C. Name resolution / symbol model *(decided)*
*Decision:* symbols are **interned + content-addressed** (like types, L3).
**Resolution is a query** ("what does `foo` resolve to in scope S?") keyed by the
scope chain + imports. A module's **exported surface = the signature-fingerprints
of its public items** (§4 / interface fingerprints) — so importing M and changing
M's *private* internals doesn't invalidate importers (early cutoff). Visibility is
part of the signature fingerprint. Falls out of the foundation; not a new beast.

### D. Derive framework — pipeline integration *(decided)*
*Decision:* `@derive`/`@expand` runs at the **comptime/expand slot — after
name-resolution** (so it has resolved enum info), **before type-checking** (so
generated code is checked like normal code, never trusted). It runs **via the
JIT** (L2). Generated AST is spliced, then **re-resolved + type-checked**.
"Expanded form of module M" is a **memoized query** run to **fixpoint** (derives
can emit more derives). Same slot as today's `@expand`, upgraded to JIT + query.

### E. Node / arena layout *(decided)*
*Decision:* **per-type arenas with typed IDs.** Each node kind (`Expr`, `Stmt`,
`Pattern`, …) gets its own dense arena; IDs are typed (`ExprId` = index into the
`Expr` arena). Cross-type references are typed ids into the right arena
(`Stmt.While` holds an `ExprId` cond + a `StmtId` body). Most cache-friendly (SoA
per kind), type-safe (ids can't be confused), aligns with per-type derived code.
Hash-consing + side-tables are per-arena, keyed by typed id.

### F. Diagnostic schema + `--fix` *(decided)*
*Decision:* a diagnostic is a structured value — `{ code, severity, primary span,
secondary spans (because-chain), message, help, fixes }`; a **Fix** =
`{ description, edits: [{ span, replacement }] }`. One structured value **renders
to human text OR JSON**; `--fix` applies the edits (non-overlapping; conflicts
skipped + reported). Content-addressed/versioned like AST-as-data. This is the
LSP/rustc model — already familiar to LLMs.

### Tier-3 resolutions
- **L1 staging:** yes — the compat shim (A) makes the all-touching change
  incremental, never a frozen tree.
- **Parallelism:** queries are pure over immutable data → the **query engine's
  scheduler** parallelizes the query DAG (work-stealing); specifics deferred.
- **Diagnostics in the query model:** a query's output is *(value + diagnostics)*;
  diagnostics memoize + invalidate *with* the query, so error quality is preserved.
- **Daemon memory:** LRU eviction of cold memoized results; specifics deferred.
- **`Value`-deletion ripple:** an explicit migration step audits std/user exposure
  of `Value` before L2 deletes it (tracked under L2 / bead `g18a`).
