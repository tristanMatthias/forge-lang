# Avra — Package Registry & Dependency Management
...
12. [avra.toml Dependency Syntax](#12-avratoml-dependency-syntax)
13. [Lockfile](#13-lockfile)
14. [Publishing](#14-publishing)
15. [Git-Based Dependencies](#15-git-based-dependencies)
16. [Avra Context Integration](#16-avra-context-integration)
...
The Avra package registry is the system for discovering, distributing, installing, and verifying third-party packages. A package is the unit of reuse in Avra — it may contain Avra source code, native libraries (Rust/Go/C), provider keyword definitions, or any combination of these.

The registry is a centralized API service maintained by the Avra project. It stores package metadata, content hashes, pre-compiled artifacts, and quality signals. Source code is hosted by package authors (typically in Git repositories) and verified against content hashes stored in the registry.

### What Makes Avra Packaging Different
...
- **No install scripts.** Nothing runs until `avra build`. The compiler is the only execution environment.
...
- **Pre-compiled artifacts.** Native providers ship as pre-compiled static libraries. Pure Avra packages ship as LLVM bitcode. First builds are fast; subsequent builds are near-instant.
- **Dead code elimination.** The compiler links only the functions you actually call. Large packages don't bloat your binary.
- **Rich machine-readable metadata.** Every package ships a Avra context — the full typed API surface — queryable without downloading source.
...
The npm ecosystem's `postinstall` scripts are the root cause of most supply chain attacks. In Avra, `avra add` downloads source and metadata. Nothing executes. The first time any package code runs is inside `avra build`, within the compiler's capability sandbox. There is no mechanism for a package to execute arbitrary code at install time.

### 2.2 Capabilities Are Compiler-Enforced

A package's manifest declares what system resources it needs — network, filesystem, native code execution. Unlike advisory systems (npm audit), the Avra compiler statically verifies these claims. If a package says `capabilities = ["network"]` but its code also touches the filesystem, the compiler refuses to build. Native code packages are verified against their `.wit` interface contracts at the FFI boundary.

### 2.3 Immutability Is Cryptographic

Once published, a version is immutable. The registry stores content hashes. The lockfile stores content hashes. `avra build` verifies hashes before compilation. If upstream source changes after you locked a version, the build fails. There is no mechanism to silently mutate a published version.
...
├── src/
│   ├── lib.av              # Avra source (the entry point for this package)
│   └── ...                 # additional Avra source files
├── native/                 # native code (optional, only for packages with native = true)
│   ├── Cargo.toml          # Rust project for native library
│   └── src/
│       └── lib.rs
├── context.av              # machine-readable API surface (auto-generated on publish)
├── tests/
│   └── ...                 # package tests
├── examples/
│   └── ...                 # runnable examples
...
- A **pure Avra package** contains only `.av` source files. It is inherently sandboxed by the language.
...
- A **keyword package** (provider) defines new syntax keywords via `keyword` declarations in its Avra source. It may or may not include native code.

The compiler adjusts its trust model based on these declarations. The user sees a single `avra add` experience regardless.

### 3.3 Package Naming

Packages are namespaced:

- `@std/*` — Standard library packages. Ship with Avra. Maintained by the core team. Cannot be published to by third parties.
...
description = "Full GraphQL server and client for Avra"
license = "MIT"
authors = ["Alice <alice@example.com>"]
repository = "https://github.com/alice/avra-graphql"
documentation = "https://avra-graphql.dev"
keywords = ["graphql", "api", "schema"]
avra_version = ">=0.2.0"               # minimum compatible Avra compiler version

[native]
enabled = true                           # this package includes native code
library = "avra_graphql"               # name of the compiled static library
...
|---|---|---|
| `network` | Makes outbound network connections | Native code audited for socket/HTTP calls; Avra code checked for `@std/http` client usage |
| `filesystem` | Reads or writes files | Native code audited for file I/O syscalls; Avra code checked for `@std/fs` usage |
| `compile_time_codegen` | Generates Avra code at compile time | Checked by compiler: does the package emit AST nodes? |
...
Avra uses semver ranges in `avra.toml`:
...
1. Read `avra.toml` for direct dependencies and their version ranges.
...
6. Write the resolved graph to `avra.lock`.
...
- **Pre-compiled artifact hosting**: LLVM bitcode (`.bc`) for pure Avra packages, static libraries (`.a`) for native packages, per target platform.
- **Quality signal computation**: test status, documentation coverage, maintenance activity, dependency health.
- **Avra context hosting**: machine-readable API surfaces for every package version, queryable without downloading source.
...
GET    /v1/packages/{name}/{version}/context  # Avra context (typed API surface)
...
```bash
avra auth login                   # authenticate with the registry
avra auth token create            # create a scoped publish token
avra auth token list              # list active tokens
avra auth token revoke <id>       # revoke a token
```

### 6.4 Rate Limiting and Abuse Prevention

The registry enforces rate limits on all endpoints. Publishing is throttled to prevent spam. Automated quality checks run on every publish (see Section 10). Malicious packages are removed by the Avra team upon report.
...
│   │   └── ...
│   ├── context/                           # Avra context files (typed API surfaces)
│   │   ├── graphql@3.1.0.av
│   │   └── ...
...
└── config.toml                            # global Avra configuration
```
...
**Tier 1 — Compilation artifacts (pinned):** Pre-compiled LLVM bitcode (`.bc`) for pure Avra packages, pre-compiled static libraries (`.a`) for native packages, type signatures, and manifests. This is everything the compiler needs to build. It is never automatically evicted. Typical size: 1-5MB per package.

**Tier 2 — Full source (evictable):** The complete source archive of each package. Only needed for reading source, running package tests, or recompiling native code from source. Can be re-downloaded on demand. Evicted by `avra cache gc`.

### 7.3 Cache Operations

```bash
avra cache status                  # show cache size breakdown
avra cache gc                      # evict Tier 2 (source archives)
avra cache gc --aggressive         # evict everything not used by current project
avra cache clear                   # wipe entire cache (requires confirmation)
avra cache prefetch                # download all deps for current project (for offline work)
```
...
|---|---|---|
| 5 pure Avra dependencies | ~8MB | 0 bytes |
| 10 deps, 3 with native code | ~25MB | 0 bytes |
| 50 deps, heavy project | ~80MB | 0 bytes |
| After `avra cache gc` | ~40% of above | 0 bytes |
...
When `avra add` fetches a native package, it downloads the pre-compiled `.a` for the current platform. No Rust/Go/C toolchain required. If no pre-compiled artifact exists for the target, the compiler falls back to building from source (requires the appropriate toolchain).

### 7.6 LLVM Bitcode Caching

Pure Avra packages are compiled to LLVM bitcode (`.bc`) by the registry at publish time. When `avra build` runs:
...
For pure Avra code, this is checked by analyzing imports and function calls against known capability-granting APIs (e.g., `@std/fs` requires `filesystem`, `@std/http` client requires `network`).
...
Capabilities are tracked per-dependency. If a **patch update** to a dependency introduces a new capability, `avra build` fails with a hard error:
...
ERROR[E0462]: capability escalation detected
  ...
  To proceed: avra allow graphql filesystem
  To rollback: avra update graphql@3.1.0
```
...
The `avra.lock` file stores SHA-256 content hashes for every resolved dependency:

```toml
[[package]]
name = "graphql"
version = "3.1.0"
source = "https://github.com/alice/avra-graphql"
...
ERROR[E0463]: content hash mismatch for "graphql" v3.1.0
  ...
  The source for this version has changed since avra.lock was created.
  This may indicate a compromised package or a registry inconsistency.

  To investigate: avra audit graphql
  To re-resolve: avra update graphql (updates lock with new hash)
```
...
```bash
avra audit --verify-log          # verify all deps against transparency log
```

### 8.6 Layer 6 — Native Code Sandboxing

Packages with `native = true` are the highest-risk category because native code can perform arbitrary operations. Avra mitigates this through:
...
2. **Users can verify pre-compiled artifacts by building from source.** `avra build --from-source` compiles all native dependencies from source and compares the output against the pre-compiled artifacts. A mismatch is a red flag.
...
4. **Reproducible builds.** Given the same lockfile and compiler version, `avra build` produces a byte-identical binary. This makes it possible to verify that a binary was built from the claimed source.

### 8.7 Security Summary

| Attack Vector | npm/PyPI Status | Avra Mitigation |
|---|---|---|
...
Avra's type system provides a complete, machine-readable public API surface for every package. The compiler uses this to enforce semantic versioning. On publish, the compiler:
...
Quality is a continuous, computed signal — not a binary badge and not a namespace. Every package in the registry has automatically computed quality metrics that are surfaced in search results and `avra info` output.
...
| **Type completeness** | Are all public exports fully typed? (Always true in Avra, but doc comments are checked.) |
...
avra search graphql
  ...
  graphql v3.1.0                     ████████░░ 8.2/10
    "Full GraphQL server and client for Avra"
...
```bash
# Add a dependency
avra add graphql                    # latest compatible version
avra add graphql@3.1.0             # specific version
avra add graphql@^3.0.0            # version range
avra add @acme/internal-auth        # organization-scoped package

# Add from Git (see Section 15)
avra add git:https://github.com/alice/avra-graphql.git
avra add git:https://github.com/alice/avra-graphql.git#v3.1.0
avra add git:https://github.com/alice/avra-graphql.git#main

# Remove a dependency
avra remove graphql

# Update dependencies
avra update                         # update all to latest compatible
avra update graphql                 # update specific package
avra update graphql@4.0.0          # update to specific version

# List dependencies
avra deps                           # show dependency tree
avra deps --flat                    # flat list
avra deps --outdated                # show available updates
```
...
```bash
# Package info
avra info graphql                   # full metadata, quality signals, capabilities
avra info graphql --context         # show typed API surface (Avra context)
avra info graphql --versions        # list all published versions
avra info graphql --changelog       # show changelog between versions

# Dependency analysis
avra deps explain http-client       # why is this in my project? full chain.
avra deps tree                      # full dependency tree with versions
avra deps size                      # size contribution of each dependency
avra deps capabilities              # all capabilities in the dependency tree
```
...
```bash
avra cache status                   # cache size breakdown by tier
avra cache gc                       # evict source archives, keep compilation artifacts
avra cache gc --aggressive          # evict everything not used by current project
avra cache clear                    # wipe entire cache
avra cache prefetch                 # download all deps for offline work
```

### 11.5 Security

```bash
avra audit                          # check all deps for known vulnerabilities
avra audit --verify-log             # verify lockfile hashes against transparency log
avra allow <package> <capability>   # approve a capability escalation
```

### 11.6 Publishing

```bash
avra publish                        # publish current package to registry
avra publish --dry-run              # show what would be published (API diff, version)
avra auth login                     # authenticate with registry
avra auth token create              # create scoped publish token
avra auth token list                # list active tokens
avra auth token revoke <id>         # revoke a token
```
...
## 12. avra.toml Dependency Syntax

Dependencies are declared in the project's `avra.toml`:
...
Dev dependencies are only resolved for `avra test` and `avra bench`. They are not included in the published package's dependency tree.

### 12.5 Capability Approvals

Approved capabilities are stored in `avra.toml` so they're version-controlled with the project:
...
When `avra add` introduces a package with capabilities, and the user approves, the approval is recorded here. Subsequent `avra build` checks approvals against this list.
...
`avra.lock` records the exact resolved dependency graph: specific versions, content hashes, and artifact hashes. It guarantees reproducible builds. The lockfile is committed to version control.

### 13.2 Format

```toml
# avra.lock — auto-generated by avra. Do not edit manually.
...
Given the same `avra.lock`, `avra.toml`, source code, and Avra compiler version, `avra build` produces a byte-identical binary. The lockfile captures everything needed for reproducibility.
...
2. **Runs tests.** `avra test` must pass. If tests fail, publish is rejected.
...
10. **Generates the Avra context** (`context.av`) — the machine-readable API surface.
...
#   ✓ context.av generated (12 exports)
#
# Ready to publish. Run `avra publish` to proceed.
```

### 14.4 Yanking

An author can yank a version, which marks it as "do not use for new installs" but does not delete it. Existing lockfiles referencing a yanked version continue to work. New `avra add` or `avra update` will not select a yanked version.

```bash
avra yank graphql@3.1.0 --reason "critical bug in schema parser"
```
...
For packages not published to the registry — private packages, forks, pre-release testing — Avra supports Git-based dependencies.
...
Git dependencies are resolved at `avra add` or `avra update` time:

1. Clone/fetch the repository to a local cache (`~/.avra/cache/git/`).
...
5. Record the resolved commit hash in `avra.lock`.

Subsequent `avra build` uses the locked commit hash. The build is reproducible as long as the commit exists in the remote repository.
...
- **Branch refs are mutable.** A `branch = "main"` dependency can change between resolves. The lockfile pins the commit hash, but `avra update` will pick up new commits.
...
## 16. Avra Context Integration

### 16.1 What Is Avra Context?

Every package ships a `context.av` file — a machine-readable description of its complete public API surface. This is the same format generated by `avra context` for project-level documentation. It includes:
...
The registry stores `context.av` for every published version. It is queryable via the API without downloading the full package:

```bash
avra info graphql --context

# graphql v3.1.0 — Avra Context
#
# Keywords:
...
The Avra context system enables:
...
- **`avra context` includes dependencies.** When you generate context for your project, it includes a summary of dependency APIs so an LLM has full context.
...
| E0465 | `lockfile_stale` | avra.lock doesn't match avra.toml (deps added/removed) |
...
All errors follow Avra's standard error format with source locations, suggestions, and machine-readable JSON output.
...
The registry index could be distributed as a Git repository — a lightweight database of package metadata, versions, hashes, and Avra contexts. The toolchain would sync this index locally, enabling fully offline search and dependency resolution.

```bash
avra registry sync              # git pull the index
avra search graphql             # queries local index — no network needed
```

### 18.2 Benefits

- Fully offline package search and resolution.
- No API dependency for `avra search` or `avra add` (only for fetching source/artifacts).
...
*End of Avra Package Registry Specification v0.1.0-draft*
